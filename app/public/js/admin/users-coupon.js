...
var myapp = (function() {
    ...
    var render = function(data) {
        data.map(function(item) {
            ...
            item.detail = item.c_detail;
            item.code = item.c_code;
            item.limit = item.c_limit;
            item.discount = item.c_discount;
            item.beginTime = Time.getTimeString(new Date(
                item.c_begin_time)).split(' ')[0];
            item.endTime = Time.getTimeString(new Date(item
                .c_end_time)).split(' ')[0];
        });
